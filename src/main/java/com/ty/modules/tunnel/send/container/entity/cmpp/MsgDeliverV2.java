...
* @date:2016下午02:54:18
 */
public class MsgDeliverV2 extends MsgHead implements MsgDeliverInterface {
	private static Logger logger= Logger.getLogger(MsgDeliverV2.class);
	...
	public static MsgDeliverV2 toObject(MsgHead msgHead, IoBuffer in){
		MsgDeliverV2 msgDeliver=new MsgDeliverV2();
		try {
...
//			fmtStr= "UTF-8";
			...
			if(msgDeliver.getRegistered_Delivery()==1){//如果为状态报告
				int destTerminalIdLength = 21;//3.0为32
			...
			byte[] reservedByte=new byte[8];
			in.get(reservedByte, 0, 8);
			msgDeliver.setReserved(new String(reservedByte));

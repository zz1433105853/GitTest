...
public class MsgSubmit extends MsgHead {
	...
	public IoBuffer toIoBuffer(){
		...
		try {
			...
			in.putString(this.feeTerminalId, 32,super.getEncoder());//Fee_terminal_Id 被计费用户的号码
			in.put(this.feeTerminalType);//Fee_terminal_type 被计费用户的号码类型，0：真实号码；1：伪码
			...
			in.putString(this.destTerminalId, 32, super.getEncoder());//Dest_terminal_Id

			in.put(this.destTerminalType);//Dest_terminal_type 接收短信的用户的号码类型，0：真实号码；1：伪码

			in.put(this.msgLength);//Msg_Length
			in.put(this.msgContent, 0, this.msgLength & 0xff);//信息内容
			in.putString(this.linkID, 20, super.getEncoder());
	...
	public static MsgSubmit toObject(MsgHead msgHead,IoBuffer in){

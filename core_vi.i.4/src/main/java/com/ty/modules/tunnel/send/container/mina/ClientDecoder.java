...
* @date:2016下午05:26:18
 */
public class ClientDecoder extends AbstractClientDecoder{
	...
	protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
		...
		case MsgCommand.CMPP_CONNECT_RESP:
			MsgConnectResp msgConnectResp = MsgConnectResp.toObject(msgHead, in);
		...
		case MsgCommand.CMPP_SUBMIT_RESP:
			MsgSubmitResp msgSubmitResp = MsgSubmitResp.toObject(msgHead, in);
		...
		case MsgCommand.CMPP_DELIVER:
			MsgDeliver msgDeliver = MsgDeliver.toObject(msgHead, in);
